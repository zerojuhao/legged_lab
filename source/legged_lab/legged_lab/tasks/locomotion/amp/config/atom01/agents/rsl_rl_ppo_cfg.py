...
@configclass
class Atom01RoughRslRlOnPolicyRunnerAmpCfg(RslRlOnPolicyRunnerCfg):
    class_name = "AMPRunner"
    num_steps_per_env = 24
    max_iterations = 10000
    save_interval = 200
    experiment_name = "atom01_amp_rough"
...
@configclass
class Atom01FlatRslRlOnPolicyRunnerAmpCfg(Atom01RoughRslRlOnPolicyRunnerAmpCfg):
    class_name = "AMPRunner"
    num_steps_per_env = 24
    max_iterations = 100000
    save_interval = 100
    experiment_name = "atom01_amp_flat"
    obs_groups = {"policy": ["policy"], "critic": ["critic"], "amp": ["amp"]}
    policy = RslRlPpoActorCriticConv2dCfg(
        init_noise_std=1.0,
        actor_hidden_dims=[512, 256, 128],
