...
def _transform_policy_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    # joint pos
    obs[:, 9:33] = _switch_joints_left_right(obs[:, 9:33])
    # joint vel
    obs[:, 33:57] = _switch_joints_left_right(obs[:, 33:57])
    # last actions
    obs[:, 57:81] = _switch_joints_left_right(obs[:, 57:81])
...
def _transform_critic_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    # joint pos
    obs[:, 12:36] = _switch_joints_left_right(obs[:, 12:36])
    # joint vel
    obs[:, 36:60] = _switch_joints_left_right(obs[:, 36:60])
    # last actions
    obs[:, 60:84] = _switch_joints_left_right(obs[:, 60:84])
